...
if FileExist("options.ini")
{
	...
	Loop parse, Text, "`n"
	{
		...
		switch ArrayText[1]
		{
			...
			case "WebAlConectarse", "WebAlDesconocer", "WebAlDesconectarse", "WebAlError", "SoundConectado", "SoundNacional", "SoundDesconectado", "SoundError", "EditConectado", "EditNacional", "EditDesconectado", "EditUser", "EditPassword", "EditDataPers", "EditWebOffAccount", "EditIniFileReadTime", "AnchoGif1", "AnchoGif2", "AnchoGif3", "AnchoGif4":
				%ArrayText[1]% := ArrayText[2]
			...
			case "LargoGif1", "LargoGif2", "LargoGif3", "LargoGif4", "GifSelected1", "GifSelected2", "GifSelected3", "GifSelected4", "GifSelectedText1", "GifSelectedText2", "GifSelectedText3", "GifSelectedText4":
				%ArrayText[1]% := ArrayText[2]
			...
			case "VoltearEntradaGif1", "VoltearSalidaGif1", "VoltearEntradaGif2", "VoltearSalidaGif2", "VoltearEntradaGif3", "VoltearSalidaGif3", "VoltearEntradaGif4", "VoltearSalidaGif4", "BarradeTareasGif1", "BarradeTareasGif2", "BarradeTareasGif3", "BarradeTareasGif4":
			{
				if (ArrayText[2] = 0 or ArrayText[2] = 1)
					%ArrayText[1]% := ArrayText[2]
				else
...
A_TrayMenu.Add("Reiniciar", MenuHandler)
Try
	A_TrayMenu.SetIcon("Reiniciar", "shell32.dll", 239)
	
A_TrayMenu.Add("Pausar", MenuHandler)
try 
	A_TrayMenu.SetIcon("Pausar", A_AhkPath, 4)

A_TrayMenu.Add()

A_TrayMenu.Add("Poner Cuenta", MenuHandler) 
Try
	A_TrayMenu.SetIcon("Poner Cuenta", "shell32.dll", 168)
	
A_TrayMenu.Disable("Poner Cuenta")
	
A_TrayMenu.Add("Quitar Cuenta", MenuHandler) 
Try
	A_TrayMenu.SetIcon("Quitar Cuenta", "shell32.dll", 136)
	
A_TrayMenu.Disable("Quitar Cuenta")

A_TrayMenu.Add()

A_TrayMenu.Add("Poner Temporizador", MenuHandler) 
Try
	A_TrayMenu.SetIcon("Poner Temporizador", "shell32.dll", 250)
	
A_TrayMenu.Disable("Poner Temporizador")

A_TrayMenu.Add("Quitar Temporizador", MenuHandler) 
Try
	A_TrayMenu.SetIcon("Quitar Temporizador", "ieframe.dll", 75)
	
A_TrayMenu.Disable("Quitar Temporizador")
	
A_TrayMenu.Add()
A_TrayMenu.Add("Configuración", MenuHandler) 

Try
	A_TrayMenu.SetIcon("Configuración", "shell32.dll", 171)
	
A_TrayMenu.Add()
A_TrayMenu.Add("Buscar Actualización", MenuHandler) 
Try
...
Try
	A_TrayMenu.SetIcon("Salir", "shell32.dll", 132)
...
MenuHandler(ItemName, ItemPos, MyMenu) {
	...
	else if (ItemName = "Reiniciar")
		Reload
	else if (ItemName = "Configuración")
	{
	    global TitteGUI
		try
		{
		...
		if (A_IconNumber = 3)
		{
			...
			try
			{
				...
				if InStr(whr.ResponseText, "Usted está conectado")
				{
					...
					if NotiNormal
						TrayTip "La cuenta se ha puesto correctamente.",, "Mute"
						
					A_TrayMenu.Disable("Poner Cuenta")
					A_TrayMenu.Enable("Quitar Cuenta")
				}
				else if NotiNormal
				{
					if InStr(whr.ResponseText, "Su tarjeta no tiene saldo disponible")
						TrayTip "La cuenta no tiene saldo.",, "Mute"
					else
						TrayTip "Hubo algun error al poner la cuenta.",, "Mute"
			...
			{
				if NotiNormal
					TrayTip "Hubo un error al poner la cuenta. Intentalo de nuevo.",, "Mute"
			}
		}
	}
	else if (ItemName = "Quitar Cuenta")
	{
						...
						if(IpResolveEtecsa != "Failed")
						{
							...
							If PingEtecsa
							{
								...
								try
								{
									...
									if NotiNormal
									{
										if InStr(whr.ResponseText, "SUCCESS")
											TrayTip "La cuenta se ha quitado correctamente.",, "Mute"
										else
											TrayTip "Hubo algun error al quitar la cuenta.",, "Mute"
									}
									...
									if (EditUser != "" or DataPers != "")
										A_TrayMenu.Enable("Poner Cuenta")	
									else
										A_TrayMenu.Disable("Poner Cuenta")	
								...
								catch
								{
									if NotiNormal
									{
										TrayTip "Hubo un error al quitar la cuenta. Intentalo de nuevo.",, "Mute"
									}
								}
							}
						}
	...
	else if (ItemName = "Poner Temporizador")
	{
		global Temporizador := 1
		global TemporizadorNotiIcon
		A_TrayMenu.Disable("Poner Temporizador")
		A_TrayMenu.Enable("Quitar Temporizador")
	...
	else if (ItemName = "Quitar Temporizador")
	{
		global Temporizador := 0
		A_TrayMenu.Enable("Poner Temporizador")
		A_TrayMenu.Disable("Quitar Temporizador")	
	...
	else if (ItemName = "Buscar Actualización")
	{
		global PopupTittle
		if (!isset(PopupTittle) or !winexist(PopupTittle))
		{
			...
			try
			{
				...
				{
					Msgbox("Esta es la ultima versión.`n`nVersion: " Version "`nAutor: AleTheKing`n`nSitio Web: https://github.com/alejandrodarz/Notificador-de-Conexion", "Buscar Actualización", "32")
				}
				else
				{
					global DarkMode
					...
					Popup.Opt("-MinimizeBox -MaximizeBox")
					Popup.MarginX := 10
					Popup.MarginY := 10
					
					Popup.Add("Link",, 'Última Versión:  <a href="' latestObj.DownloadURLs[1] '">' latestObj.version '</a>')
					
					
					Popup.Add("Text","y+2 xm", "Versión Actual: ")
					
					TextPopupVI := Popup.Add("Text", "yp x+1", Version)
					TextPopupVI.SetFont("underline")
					
					
					Popup.Add("Link","xm", '<a href="https://github.com/alejandrodarz/Notificador-de-Conexion/releases/tag/' latestObj.version '">Cambios:</a>')

					
					Popup.Add("Edit", "w200 h80 ReadOnly", latestObj.change_notes)
					
					Popup.Add("Link",, '<a href="https://github.com/alejandrodarz/Notificador-de-Conexion">Ir al Sitio Web</a>')
					global PopupTittle := Popup.Title := "Buscar Actualización"
				
					Popup.Title := "Buscar Actualización"
					
					Popup.Add("Text",, "¿Deseas actualizar a la ultima versión?")
			...
			{
				Msgbox("Verifique que su conexió a internet sea estable.`n`nVersion: " Version "`nAutor: AleTheKing`n`nSitio Web: https://github.com/alejandrodarz/Notificador-de-Conexion", "Buscar Actualización", "16")
			}
		}
		else
...
{	
	...
	TitteGUI := MyGui.Title := "[CFG] Notificador de Conexión"
	
	MyTab := MyGui.Add("Tab3",, ["General","Opciones","GIF"])
	;////////////////////////  GroupBox [Generales]
	
	MyGroupBoxGenerales := MyGui.Add("GroupBox","w250 h150 Section", "Básicas")
	...
	MyCheckIniciarConWindowsText := MyGui.Add("Text","x+-1 yp", "Iniciar automaticamente con el sistema.")
	...
	MyCheckDarkModeText := MyGui.Add("Text","x+-1 yp", "Habilitar modo oscuro.")
	...
	;////// [Verificar conexión]
	MyGui.Add("Text","xs+15 y+15", "Verificar conexión cada:")
	...
	;////// [Verificar ping]
	MyGui.Add("Text","xs+15 y+15", "Verificar ping cada:")
	...
	GMyUpDownVerificarConx(*)
	{
		if (MyUpDownVerificarConx.value != 1)
			MyTextVerificarConx.value := "veces."
		else
			MyTextVerificarConx.value := "vez."
	}
	;//////
	...
	;////// [Acerca de:]
	MyGui.Add("Text","xp-36 y+14", "Acerca de:")
	...
	;////////////////////////  GroupBox [Ping]
	MyGui.Add("GroupBox", "xs w250 h210 Section", "Ping")
	
	;////// [Ping a Internet]
	MyCheckWebConectado := MyGui.Add("Checkbox","xs+15 yp+25 Section vMyCheckConectado", " ")
	MyCheckWebConectado.OnEvent("Click", GMyCheckWebConectado)
	MyCheckWebConectado.value := WebConectado
	
	MyCheckWebConectadoText := MyGui.Add("Text","x+-0.5 yp", "Ping a Internet")
	...
	;////// [Ping a Intranet]
	MyCheckWebDesconocido := MyGui.Add("Checkbox","xs y+20 vMyCheckDesconocido", " ")
	MyCheckWebDesconocido.OnEvent("Click", GMyCheckWebDesconocido)
	MyCheckWebDesconocido.value := WebDesconocido
	
	MyCheckWebDesconocidoText := MyGui.Add("Text","x+-0.5 yp", "Ping a Intranet")
	...
	;////// [Ping al Portal]
	MyCheckWebDesconectado := MyGui.Add("Checkbox","xs y+20 vMyCheckDesconectado", " ")
	MyCheckWebDesconectado.OnEvent("Click", GMyCheckWebDesconectado)
	MyCheckWebDesconectado.value := WebDesconectado
	
	MyCheckWebDesconectadoText := MyGui.Add("Text","x+-0.5 yp", "Ping al Portal")
	...
	MyGui.Add("GroupBox","y" YMyGroupBoxGenerales " x" (XMyGroupBoxGenerales+WMyGroupBoxGenerales+MyGui.MarginX) " w260 h150 Section", "Cuenta")
	
	;////// [Usuario]
	MyEditUserText := MyGui.Add("Text","xs+15 yp+30", "Usuario:")
	MyEditUser := MyGui.Add("Edit","xp yp+18 w230 h22 vMyEditUser")
	MyEditUser.SetFont("cBlack")
	MyEditUser.value := EditUser
	;//////
	
	
	;////// [Contrasena]
	MyEditPasswordText := MyGui.Add("Text","xp yp+30", "Contraseña:")
	MyEditPassword := MyGui.Add("Edit","xp w230 h22 vMyEditPassword Password")
	MyEditPassword.SetFont("cBlack")
	MyEditPassword.value := EditPassword
	;//////
	
	
	;////////////////////////  GroupBox [Notificaciones]
	
	MyGui.Add("GroupBox", "xs w260 h210 Section", "Notificaciones")
	...
	MyTextGIF := MyGui.Add("Text","xp+99 yp+25", "GIF")
	MyTextGIF.SetFont("underline")
	MyTextGIF.OnEvent("Click", GMyTextGIF)
	
	MyTextWindows := MyGui.Add("Text","x+12 yp", "Windows")
	MyTextWindows.SetFont("underline")
	MyTextWindows.OnEvent("Click", GMyTextWindows)
	
	
	MyTextSonido := MyGui.Add("Text","x+18 yp", "Sonido").SetFont("underline")
	
	;////// [Conectado]
	MyCheckNotiConectadoText := MyGui.Add("Text","xs+15 yp+30", "Conectado:")
	...
	;////// [Desconocido]
	MyCheckNotiNacionalText := MyGui.Add("Text","xs+15 yp+35", "Desconocido:")
	...
	;////// [Desconectado]
	MyCheckNotiDesconectadoText := MyGui.Add("Text","xs+15 yp+35", "Desconectado:")
	...
	;////// [Error]
	MyCheckNotiErrorText := MyGui.Add("Text","xs+15 yp+35", "Error:")
	...
	MyTextNotiNormal := MyGui.Add("Text","yp x+-1", "Notificaciones básicas y de información.")
	...
	MyGroupBoxPaginasWeb := MyGui.Add("GroupBox", "w250 h275 Section", "Paginas Web")
	...
	MyCheckAbrirWebAlConectarseText := MyGui.Add("Text","x+-0.5 yp", "Abrir web si hay Internet")
	...
	MyCheckAbrirWebAlDesconocerText := MyGui.Add("Text","x+-0.5 yp", "Abrir web si hay Intranet")
	...
	MyCheckAbrirWebAlDesconectarseText := MyGui.Add("Text","x+-0.5 yp", "Abrir web si hay Portal")
	...
	MyCheckAbrirWebAlErrorText := MyGui.Add("Text","x+-0.5 yp", "Abrir web si no hay Conexión")
	...
	MyCheckWebOptionText := MyGui.Add("Text","x+-0.5 yp-5", "Abrir webs siempre la primera vez y`ncuando el explorador no esté abierto.")
	...
	GTextInfoWeb(*)
	{
		...
		Result := Msgbox("Se abrirá la pagina web como mínimo una vez, siempre, esté o no esté abierto el Explorador Web, despues de la primera vez, si el Explorador Web no esta abierto abrirá la pagina web y si está abierto pues no abrira la pagina web. Esta opcion afecta a todas las opciones de abrir las paginas web y cada opcion es independiente del resto", "Info", "32")
		if Result
		{
			if DarkMode
				MyTextInfoWeb.SetFont("cAqua")
	...
	;////////////////////////  GroupBox [Avanzado]
	MyGui.Add("GroupBox","y" YMyGroupBoxPaginasWeb " x" (XMyGroupBoxPaginasWeb+WMyGroupBoxPaginasWeb+MyGui.MarginX) " w260 h275 Section", "Avanzado")
	
	;////// [Web para poner la cuenta]
	MyGui.Add("Text","xs+15 yp+25", "Web para poner la cuenta:")
	...
	MyCheckDataPersText := MyGui.Add("Text","x+-0.5 yp", "Personalizar datos requeridos")
	...
	MyCheckWebOffAccountText := MyGui.Add("Text","x+-0.5 yp", "Web para quitar la cuenta")
	...
	MyCheckIniFileReadTimeText := MyGui.Add("Text","x+-0.5 yp", 'Utilizar "InternetAccount.ini"')
	...
	MyGroupBoxTemporizador := MyGui.Add("GroupBox", "x" XMyGroupBoxPaginasWeb " y" (YMyGroupBoxPaginasWeb+HMyGroupBoxPaginasWeb+MyGui.MarginY) " w" (510+MyGui.MarginX) " h85 Section", "Temporizador")
	...
	MyCheckTemporizador := MyGui.Add("Checkbox","xs+15 yp+20 Section vTemporizador", " ")
	MyCheckTemporizador.value := Temporizador
	
	MyTextTemporizador := MyGui.Add("Text","x+-0.5 yp", "Quitar la cuenta al cabo de:")
	...
	MyTextTempNotiIcon := MyGui.Add("Text","x+-0.5 yp", "Mostar cuenta atras en el Icono")
	...
	MyTextTempNotiTooltip := MyGui.Add("Text","x+-0.5 yp", "Mostar cuenta atras en la Info del Icono")
	...
	MyTextTempNotiWindows := MyGui.Add("Text","x+-0.5 yp", "Mostar cuenta atras en una Notificación")
	...
	;//////////////////////// GroupBox [Perfil]
	MyGui.Add("GroupBox", "w520 h45 Section", "Perfil")
	
	
	MyRadioGroupConectadoGif := MyGui.Add("Radio", "xs+15 yp+20 vConectadoGif", "Conectado")
	MyRadioGroupConectadoGif.value := ConectadoGif
	MyRadioGroupConectadoGif.OnEvent("Click", GMyRadioGroup) 
	MyRadioGroupDesconocidoGif := MyGui.Add("Radio", "yp x+60 vDesconocidoGif", "Desconocido")
	MyRadioGroupDesconocidoGif.value := DesconocidoGif
	MyRadioGroupDesconocidoGif.OnEvent("Click", GMyRadioGroup) 
	MyRadioGroupDesconectadoGif := MyGui.Add("Radio", "yp x+60 vDesconectadoGif", "Desconectado")
	MyRadioGroupDesconectadoGif.value := DesconectadoGif
	MyRadioGroupDesconectadoGif.OnEvent("Click", GMyRadioGroup) 
	MyRadioGroupErrorGif := MyGui.Add("Radio", "yp x+60 vErrorGif", "Error")
	...
	;//////////////////////// GroupBox [Ajustes]
	MyGroupBoxAjustes := MyGui.Add("GroupBox", "xs w250 h132 Section", "Ajustes")
	...
	MyTextBarradeTareasGif := MyGui.Add("Text","x+-0.5", "Activar Barra de Tareas al iniciar GIF")
	MyTextBarradeTareasGif.OnEvent("Click", GBarradeTareasGif)
	;////// 
	
	;////// [Corrección de posición del GIF verticalmente]
	WinGetPos ,,, &H, "ahk_class Shell_TrayWnd"
	MyTextPosVGif := MyGui.Add("Text","xs+15 yp+22", "Posición del GIF verticalmente:")
	...
	MyTextRelacionGif := MyGui.Add("Text","x+-0.5 yp", "Relación de aspecto.")
	...
	;//////////////////////// GroupBox [Efectos]
	MyGui.Add("GroupBox", "xs w250 h180 Section", "Efectos")
	
	Efectos := ["Desde el Centro", "Desplazándose",  "Desplazándose Rápido", "Hacia Izquierda", "Hacia Izquierda Desplazándose", "Hacia Derecha", "Hacia Derecha Desplazándose", "Hacia Abajo", "Hacia Abajo Desplazándose", "Hacia Arriba", "Hacia Arriba Desplazándose", "Desde Esquina Superior Izquierda", "Desde Esquina Superior Izq. Desp.", "Desde Esquina Superior Derecha", "Desde Esquina Superior Der. Desp.", "Desde Esquina Inferior Izquierda", "Desde Esquina Inferior Izq. Desp.", "Desde Esquina Inferior Derecha", "Desde Esquina Inferior Der. Desp."]
	
	;////// [Entrada]
	MyGui.Add("Text","xs+15 yp+20", "Inicio:")
	...
	MyGui.Add("Text","xs+15 y+10", "Duración:")
	...
	MyTextVoltearEntradaGif := MyGui.Add("Text","x+-0.5", "Voltear GIF")
	...
	;////// [Salida]
	MyGui.Add("Text","xs+15 y+25 ", "Salida:")
	...
	MyTextVoltearSalidaGif := MyGui.Add("Text","x+-0.5", "Voltear GIF")
	...
	;////// [Tiempo en Pantalla:]
	MyGui.Add("Text","xs+15 y+25", "Tiempo en Pantalla:")
	...
	MyLV := MyGui.Add("ListView", "y" (YMyGroupBoxAjustes+5) " x" (XMyGroupBoxAjustes+WMyGroupBoxAjustes+MyGui.MarginX ) " w260 h312 Icon", ["Name","In Folder","Size (KB)","Type"])
	...
	MyGui.Add("GroupBox","x" XMyGroupBoxTemporizador " y" (YMyGroupBoxTemporizador+HMyGroupBoxTemporizador+MyGui.MarginY) " w520 h50 Section")
	MyGui.Add("Button", "Default w120 xs+10 yp+16", "Guardar").OnEvent("Click", Gui_BGuardar)
	MyGui.Add("Button", "Default w120 yp", "Cancelar").OnEvent("Click", Gui_BClose)
	MyGui.Add("Button", "Default w120 yp", "Restablecer").OnEvent("Click", Gui_BRestablecer)
	MyGui.Add("Button", "Default w120 yp", "Probar").OnEvent("Click", Gui_BProbar)
	...
	MyGui.Add("GroupBox","x" XMyGroupBoxTemporizador " y" (YMyGroupBoxTemporizador+HMyGroupBoxTemporizador+MyGui.MarginY) " w520 h50 Section")
	MyGui.Add("Button", "Default w160 xs+10 yp+16", "Guardar").OnEvent("Click", Gui_BGuardar)
	MyGui.Add("Button", "Default w160 yp", "Cancelar").OnEvent("Click", Gui_BClose)
	MyGui.Add("Button", "Default w160 yp", "Restablecer").OnEvent("Click", Gui_BRestablecer)
	...
	GTextInfo(*)
	{
		...
		Result := Msgbox("Version: " Version "`nAutor: AleTheKing`n`nSitio Web: https://github.com/alejandrodarz/Notificador-de-Conexion", "Info", "32")
		if Result
		{
			if DarkMode
				MyTextInfo.SetFont("cAqua")
	...
	MyGui.Opt("-DPIScale")
	MyGui.Show("Center h480")

	
	Gui_BRestablecer(*)
	{
		Result := MsgBox("¿Quieres restablecer todo?", TitteGUI, "YesNo")
		if Result = "Yes"
		{
			MyCheckIniciarConWindows.value := 0
			MyCheckDarkMode.value := 0
	...
	Gui_BGuardar(*)
	{
		Result := MsgBox("¿Quieres guardar los cambios?", TitteGUI, "YesNo")
		if Result = "Yes"
		{
			...
			options .= "EditUser=" SaveData.MyEditUser "`n"
			options .= "EditPassword=" SaveData.MyEditPassword "`n"
			...
			options .= "VoltearSalidaGif4=" VoltearSalidaGif4
		...
		if (A_IconNumber != 1 and A_IconNumber != 2 and A_IconNumber != 3)
		{
			...
			While(PosXOld = OutputVarX and PosYOld = OutputVarY)
			{
				if ToolTip1
				{
					...
					if InStr(A_IconTip, "Se ha establecido la conexión a Internet")
					{
						A_IconTipnew := "Se ha establecido la conexión a Internet.`nCuenta: " UserNameAccount  "`nTiempo: " TimeAccount
					}
					else
						A_IconTipnew := "Se ha establecido una conexión que no es Internet.`nCuenta: " UserNameAccount  "`nTiempo: " TimeAccount
				...
				if ToolTip2
				{
					...
					A_IconTip := AIconTipold "`nTemporizador: " FormatSeconds(TimeRestTemp)
				}
	...
	if !winexist(TitteGUI) or !TitteGUI
	{
					...
					if Ping
					{
						if (w != EditDesconectado)
						{
							...
							{ 
								if (A_IconNumber != 5 and A_IconNumber != 6 and A_IconNumber != 7 and A_IconNumber != 8 and A_IconNumber != 9 and A_IconNumber != 10)
								{
									...
									A_TrayMenu.Enable("Buscar Actualización")
										
									A_IconTip := "Se ha establecido la conexión a Internet."
												...
												{	
													...
													If PingEtecsa
													{
														...
														try
														{
															...
															{
																A_IconTip := "Se ha establecido la conexión a Internet.`nCuenta: " usernamearray[2]  "`nTiempo: " whr.ResponseText
									...
									if IniFileReadTime
									{
										...
										try
										{
											if FileExist("InternetAccount.ini")
											{
												...
												A_IconTip := "Se ha establecido la conexión a Internet.`nCuenta: " Account "`nTiempo: " Horas
								...
								if Temporizador
								{
									if (!isset(SecondsTemp) or SecondsTemp = 0)
									{
										...
										if (SecondsTemp = 0)
										{
											Temporizador := 0
											if NotiNormal
												TrayTip "El tiempo establecido debe de ser mayor que 0.","Temporizador:", "Mute"
											A_TrayMenu.Enable("Poner Temporizador")
											A_TrayMenu.Disable("Quitar Temporizador")
										}
										else
									...
									else
									{
										...
										if TemporizadorNotiWindows
										{
											...
											{
												TrayTip FormatSeconds(SecondsTemp),"Temporizador:", "Mute"
												NotificationTemp := 1
											}
											else 
											{
												...
												if (DivisionPartTemp != 0)
												{
													if (NotificationTemp != 4 and NotificationTemp != 5)
													{
														...
														{
															TrayTip FormatSeconds(SecondsTemp-ElapsedTimeTemp),"Temporizador:", "Mute"
															NotificationTemp += 1
														}
													}
													else if (NotificationTemp = 4)
													{
														...
														{
															TrayTip "La cuenta atras está al acabarse.`n" FormatSeconds(SecondsTemp-ElapsedTimeTemp),"Temporizador:", "Mute"
															NotificationTemp += 1
														}
													}
													else if (NotificationTemp = 5)
													{
														...
														{
															TrayTip "La cuenta atras finalizó con éxito!.`n" FormatSeconds(0),"Temporizador:", "Mute"
															NotificationTemp := 0
														}
													}
												}
												...
												{	
													if NotiNormal
														TrayTip "El temporizador tiene un tiempo muy pequeño para poder activar las notificaciones.","Temporizador:", "Mute"
										...
										if TemporizadorNotiIcon
										{
											...
											{
												if NotiNormal
													TrayTip "El temporizador tiene un tiempo muy pequeño para poder activarlo en el Icono.","Temporizador:", "Mute"
										...
										if (ElapsedTimeTemp >= SecondsTemp)
										{
											...
											MenuHandler("Quitar Cuenta", 4, A_TrayMenu)
											A_TrayMenu.Enable("Poner Temporizador")
											A_TrayMenu.Disable("Quitar Temporizador")
										}
									}
							...
							} else 
							{
								if (A_IconNumber != 4 and A_IconNumber != 11 and A_IconNumber != 12 and A_IconNumber != 13 and A_IconNumber != 14 and A_IconNumber != 15)
								{
									...
									A_IconTip := "Se ha establecido una conexión que no es Internet."
						...
						} else 
						{
							if (A_IconNumber != 3)
							{
								...
								A_IconTip := "Se ha detectado el portal cautivo."
								...
								if Temporizador
								{
									...
									SecondsTemp := 0
									if NotiNormal
										TrayTip "Se ha cancelado la cuenta atras.","Temporizador:", "Mute"
								}
									...
									try
									{
										if FileExist("InternetAccount.ini")
										{
											...
											if (Saldo = "No tiene saldo") 
												A_IconTip := "Se ha detectado el portal cautivo.`nCuenta: " Account "`nTiempo: " Saldo
		...
		if !Ping
		{
			if (A_IconNumber != 2)
			{
				...
				A_IconTip := "Se ha perdido la conexión."
				...
				if NotiError
					TrayTip "Se ha perdido la conexión.",, "Mute"
	...
	if (DarkMode)
	{
		switch uMsg
		{
			...
			case WM_CTLCOLORSTATIC:
			{
				...
				if (lParam != HwndMyTextInfoWeb and lParam != HwndMyTextInfo)
				{
